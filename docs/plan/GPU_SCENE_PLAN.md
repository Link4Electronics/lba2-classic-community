...
Status: Phase 2 Implemented (interior room bricks, bodies and NZW polys behind `GpuScene`, default off). Phase 3 is In Progress — interior brick iso-depth, the scene-interior mark, opaque world sprites (`GpuRenderer_DrawSprite`: TYPE_OBJ_SPRITE / TYPE_OBJ_ANIM_3DS / 1:1 TYPE_EXTRA), and the interior `Screen` re-blit plate (`AffGrille_BuildScreenPlate` + `DefaultBoxOneClear` under `GpuRenderer_IsSceneInterior`) have landed; HUD/text, blend sprites (`ScaleSpriteTransp` / `PtrTransPal`), readback deletion and the R8 present conversion are still Proposed. [RENDERER.md](../RENDERER.md) remains the reference for what the engine does today.
...
- Interior shadows need no depth: the original interior `ShadeSpan` is a flat darken (`ShadeBoxBlk`) with no Z test, unlike the exterior's depth-tested spans. The split is pixel-preserving: the same spans darken the FBO (where the room now lives) through the existing `GpuRenderer_Shadow*` machinery in a new no-depth mode, while `ShadeBoxBlkOverlay` darkens whatever non-zero actor pixels sit in the Log (index 0 is the overlay's transparent hole — left alone so the FBO shows through; plain `ShadeBoxBlk` would map 0 through opaque `clut[0]` and paint solid holes) — each buffer receives exactly the darken its pixels received when they shared one buffer.
...
**Phase 2 — interior bodies and NZW polys to the FBO (Implemented).** The `AffOneObject` / `TYPE_DART` gates in [OBJECT.CPP](../../SOURCES/OBJECT.CPP) open the 3D pass for `CUBE_INTERIEUR` while the flag is on (today's exterior-only gate gains `GpuScene AND CubeMode == CUBE_INTERIEUR`), and COMMON's intake keys on "the FBO has room content this frame": `GpuRenderer_RenderTriangleList`'s `s_fboHasTerrain` condition gains its interior sibling `s_fboHasRoom`, set when `GpuRenderer_DrawBrick` successfully enqueues a quad, cleared by `GpuRenderer_ClearFBO` and the lifecycle resets that already clear `s_fboHasTerrain`. No depth is required — interior bodies use `TYPE_ISO` so `GetZO3`/`GetZO4` leave `Pt_ZO` alone, the cleared depth stays farthest, and front-wall re-blits still land in the Log above the bodies. The `AFF_ALL` path snapshots the clean room and the `AFF_OBJETS` head restores it (no `ReadbackDepthBuffer`, no `DrawAnimatedPolys` — exterior-only), so static frames start from the room rather than the previous frame's bodies. Success: screenshots match, including an actor standing in a doorway with a wall in front.

**Phase 3 — sprites and text to the FBO, readback deleted (In Progress).** Opaque world sprites first: `GpuRenderer_DrawSprite` packs a single AffGraph or 1:1 ScaleSprite entry into a separate RG8 page (same mode-7 fragment path; `GpuDrawParams.spriteAtlas` selects brick vs sprite bind) and submits a screen-space quad. Interior takes no depth-test/write (software `AffGraph` never Z-tests; front walls stay `DrawRecover3`); exterior `GEQUAL`-tests terrain depth with no depth-write. Hooks sit in `AffOneObject` only — `TYPE_OBJ_SPRITE`, `TYPE_OBJ_ANIM_3DS`, and the opaque branch of `TYPE_EXTRA` — so HUD sites (`COMPORTE`, `INVENT`, `INCRUST`, `GAMEMENU`) stay in Log; scaled and `EXTRA_TRANSPARENT` sprites stay software. On success the clipped opaque footprint in Log is punched to index 0 so the present shows the FBO quad; interior still runs `DrawRecover3` for the front-wall re-blit. The interior `Screen` plate (`AffGrille_BuildScreenPlate` + `DefaultBoxOneClear` under `GpuRenderer_IsSceneInterior`) keeps `CopyMask`'s wall re-blit and `BoxClean` correct once the room leaves `Log`. Still open: font/HUD quads, blend sprites, skipping `GpuRenderer_ReadbackDepthBuffer` and its consumers (`ZBufBoxOverWrite2`'s color-0 hole trick), and the staging-upload shrink. A companion sub-phase, independent and byte-identical by construction, converts the Log present to an R8 palette-index texture with the LUT lookup in the present shader, deleting the CPU ARGB conversion for every screen that still uses the Log.
