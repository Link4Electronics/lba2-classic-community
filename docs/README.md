...
| [CRASH_INVESTIGATION.md](CRASH_INVESTIGATION.md) | Runbook for a crash you already have: symbolizing a player's `CRASH` block, ASan builds (preload or static link), inspecting state at the fault, reading ASan output, pointer-arithmetic traps, and pinning the fix with a regression test. |
...
| [RENDERER.md](RENDERER.md) | Hardware-accelerated render path: the behavior-owning common layer and the GL / SDL3-GPU backends behind a vtable, shared shaders, build flags and status. |
...
| [CRASH_REPORT_PLAN.md](plan/CRASH_REPORT_PLAN.md) | Implemented | Recording native crashes in `adeline.log` on all four platforms and keeping the previous run's log: the decisions taken, the research, prototype results from macOS, Linux and Android, the design per platform, and where the implementation diverged from it once measured. See [the logging concept](knowledge/subsystems/logging.md) for what the engine does today. Tracks #680. |
...
| [RECORDER_OBSERVER_REVIEW.md](plan/RECORDER_OBSERVER_REVIEW.md) | Research | Architecture review of the session recorder against what it is for: the recorder is an observer, a scheduler and a session manager in one, and only the first can be passive. Holds the perturbation ledger in one place, argues the digest needs a three-category membership rule that a contributor had already written out by hand, reads Doom's demo system for what it settles and the one place it argues back, argues for preferring the reading that has no preconditions, argues the recorder's value is entirely in being believed and prices the gate that would make it so (its general lessons live in [BUG_HUNTING.md](BUG_HUNTING.md)), prices the entry-condition gate at a prefix test, argues build identity belongs in its own header lines rather than welded into `build.flags`, and prices step A of the engine-tick ladder, which deadlocks as the ladder states it because the modal loops take their clock from the present, and records four figures withdrawn to a replay arm that had no game to replay into. Companion to [RECORDING_RESEARCH.md](plan/RECORDING_RESEARCH.md). |
...
| [GPU_SCENE_PLAN.md](plan/GPU_SCENE_PLAN.md) | Phase 3 In Progress | Moving scene drawing from the software Log path onto the GL / SDL3 GPU backends behind one flag: interior room quads (phase 1), interior bodies (phase 2), then opaque world sprites (phase 3, landed — HUD, blend sprites, readback deletion still open), all batched through GPURENDERER_COMMON. Menus and text are evaluated last and expected to stay software. |
